...
const Delete = ({ userDetails }: Incoming) => {
  ...
  const handleDelete = async (e: any) => {
    ...
    const userConfirm = prompt(
      `You are about to delete your account. Enter ${userDetails.username} to proceed.`
    );
    ...
    const confirmUserRequest = confirm(
      "Are you sure you wish to delete this account. It can't be reversed!"
    );
  ...
  return (
    <>
      ...
      <section className={styles.container}>
        ...
        <p className={styles.info}>Below are your details..</p>
        <hr />
        <p className={styles.detail}>
          Username: <span>{userDetails?.username}</span>
        </p>
        <p className={styles.detail}>
          Email Address: <span>{userDetails?.emailAddress}</span>
        </p>
        <p className={styles.detail}>
          Account Created:{" "}
          <span>
            {moment(userDetails?.createdAt).format("MMMM Do YYYY, h:mm a")}
          </span>
        </p>
